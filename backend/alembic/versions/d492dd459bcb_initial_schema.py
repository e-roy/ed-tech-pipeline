...
NOTE: This migration assumes the frontend has already created the auth_user table
via NextAuth/Drizzle. The backend does NOT manage the auth_user table.
"""
...
def upgrade() -> None:
    """Upgrade schema."""
    # NOTE: auth_user table is created by the frontend (NextAuth/Drizzle)
    # The backend only reads from it, does not create or manage it.

    # Sessions table - references auth_user.id (managed by frontend)
    op.create_table('sessions',
    sa.Column('id', sa.String(length=255), nullable=False),
    sa.Column('user_id', sa.String(length=255), nullable=False),  # UUID string from auth_user
    ...
    # WebSocket connections table - no FK constraint for flexibility
...
def downgrade() -> None:
    ...
    # NOTE: Do NOT drop auth_user - it's managed by the frontend
