...
3. **Configure Integration**:
   ...
   - Integration URL: `http://<ec2-ip>:8000/ws/{session_id}`
   - For `$connect`: Forward connection to backend
...
- Update `NEXT_PUBLIC_API_URL` from `http://<ec2-ip>:8000` to REST API Gateway URL
  - Example: `https://abc123.execute-api.us-east-2.amazonaws.com/prod`
- Update `NEXT_PUBLIC_WS_URL` from `ws://<ec2-ip>:8000` to WebSocket API Gateway URL
  - Example: `wss://def456.execute-api.us-east-2.amazonaws.com/prod`

**CORS Configuration** (`backend/app/main.py`):
- Update `FRONTEND_URL` in `.env` to match Vercel deployment URL
- Ensure CORS allows requests from API Gateway domain (if needed)
...
- Verify `wss://<websocket-api-gateway-url>/ws/{session_id}` connects
...
### WebSocket Endpoints (No Changes Needed)

- `WS /ws/{session_id}` - Real-time agent progress updates
- Already optimized for long-running connections
...
1. **Revert Frontend Environment Variables**:
   - Change `NEXT_PUBLIC_API_URL` back to `http://<ec2-ip>:8000`
   - Change `NEXT_PUBLIC_WS_URL` back to `ws://<ec2-ip>:8000`
...
- [ ] WebSocket API Gateway connects: `wss://<websocket-url>/ws/{session_id}` establishes connection
...
- **WebSocket API Gateway setup**: 1 hour
- **Security group configuration**: 30 minutes
...
2. Vercel: Environment variables - Frontend API URLs
3. AWS: API Gateway configurations (via console or CloudFormation)
4. EC2: Security group rules (if needed)
